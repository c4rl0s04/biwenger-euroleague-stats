import { normalizePlayerName } from '../../../api/euroleague-client.js';
...
export class PlayerMatcher {
  ...
  /**
   * @param {Object} elPlayer - { code: '123', name: 'NAME, SURNAME', teamCode: 'MAD' }
   * @param {Array} biwengerPlayers - Array of all DB players
   * @param {number|null} elTeamId - Biwenger Team ID if known
   * @returns {Promise<Object|null>} Matched Biwenger Player or null
   */
  async findMatch(elPlayer, biwengerPlayers, elTeamId) {
    ...
    const existingLink = biwengerPlayers.find((p) => p.euroleague_code === elCode);
    if (existingLink) {
    ...
    if (elTeamId) {
      ...
      const candidates = biwengerPlayers.filter(
        (p) => p.team_id === elTeamId && !p.euroleague_code
      );
      ...
      if (candidates.length > 0 && process.stdin.isTTY) {
        try {
          ...
          const choices = candidates.map((c) => ({ title: `${c.name} (ID: ${c.id})`, value: c }));
