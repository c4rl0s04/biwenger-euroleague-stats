...
function seedMockData() {
  ...
  const stats = db
    .prepare('SELECT id, fantasy_points FROM player_round_stats WHERE valuation IS NULL')
    .all();

  const updateValuation = db.prepare(
    'UPDATE player_round_stats SET valuation = @val WHERE id = @id'
  );
  ...
  const matches = db
    .prepare('SELECT rowid, home_score, away_score FROM matches WHERE home_score IS NULL')
    .all();
  const updateMatch = db.prepare(
    "UPDATE matches SET home_score = @h, away_score = @a, status = 'finished' WHERE rowid = @id"
  );
  ...
  `);
  const updateMatchIDs = db.prepare(
    'UPDATE matches SET home_id = @hid, away_id = @aid WHERE rowid = @id'
  );
  ...
  db.transaction(() => {
    ...
    const matchesToUpdate = db
      .prepare(
        'SELECT rowid, home_team, away_team FROM matches WHERE home_id IS NULL OR away_id IS NULL'
      )
      .all();
