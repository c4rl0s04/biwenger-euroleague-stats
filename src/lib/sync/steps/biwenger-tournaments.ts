import { fetchTournament, fetchRoundsLeague } from '../../api/biwenger-client';
...
export async function run(manager: SyncManager) {
  ...
  for (const round of allRounds) {
    // Future rounds are included because they may reveal upcoming tournaments.
  ...
  for (const tId of Array.from(tournamentIds)) {
    ...
    try {
      const tData = await fetchTournament(Number(tId));
      ...
      await tournamentMutations.upsertTournament({
        id: data.id,
        league_id: Number(CONFIG.API.LEAGUE_ID),
        name: data.name,
        type: data.config?.mode || data.type || 'unknown',
        status: (status === 'finished' ? 'finished' : 'active') as any,
      ...
      if (data.rounds && Array.isArray(data.rounds)) {
        ...
        // -- A. Upsert Phases --
        const phasesMap = new Map<string, number>(); // type -> db_id
