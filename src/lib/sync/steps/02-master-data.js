...
export async function run(manager) {
  ...
  const getSimilarity = (name1, name2) => {
  ...
  for (const club of clubs) {
    ...
    let dbTeam = biwengerTeams.find((t) => t.name.toLowerCase() === club.name.toLowerCase());
    ...
    if (!dbTeam) {
      ...
      // Intersection: Maccabi, Tel, Aviv (3). Union: Maccabi, Playtika, Rapyd, Tel, Aviv (5). Score: 0.6
      // "Virtus Segafredo Bologna" (3) vs "Virtus Bologna" (2). Int: 2. Union: 3. Score: 0.66
      if (maxScore >= 0.4) {
        dbTeam = bestMatch;
        manager.log(
