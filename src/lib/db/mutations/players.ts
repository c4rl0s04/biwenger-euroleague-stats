  ...
  return {
    ...
    upsertTeam: async (params: UpsertTeamParams) => {
      await db.query(
        ...
        ON CONFLICT(id) DO NOTHING
      `,
        [params.id, params.name, params.short_name, params.img]
      );

      await db.query(
        `
        INSERT INTO team_seasons (season_id, team_id, name, short_name, img, updated_at)
        VALUES ($1, $2, $3, $4, $5, NOW())
        ON CONFLICT(season_id, team_id) DO UPDATE SET
          name = excluded.name,
          short_name = excluded.short_name,
          img = COALESCE(team_seasons.img, excluded.img),
          updated_at = NOW()
      `,
        [seasonId, params.id, params.name, params.short_name, params.img]
      );
    },
  };
}
