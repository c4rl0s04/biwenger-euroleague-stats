...
export const DEFAULT_THEME_PREFERENCE: ResolvedTheme = 'dark';
...
export function applyTheme(document: Document, resolvedTheme: ResolvedTheme) {
  ...
  // React uses hoisted meta content to match nodes during hydration. Leave the
  // static dark no-JS fallback intact; own a first, unconditional runtime override.
...
var t='${DEFAULT_THEME_PREFERENCE}';try{var s=localStorage.getItem(${JSON.stringify(THEME_STORAGE_KEY)});if(s==='system'||s==='dark'||s==='light')t=s;}catch(e){}
