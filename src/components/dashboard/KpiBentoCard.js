...
export default function KpiBentoCard({
  ...
  return (
    <div
      className={`glass-panel rounded-2xl p-5 transition-all duration-300 group ${style.border} ${className}`}
    >
      {/* Header: Icon and Title */}
      <div className="flex items-start justify-between mb-4">
        <div
          className={`p-2.5 rounded-xl ${style.iconBg} transition-colors group-hover:scale-110 duration-300`}
        >
      ...
      {/* Footer / Trend Label */}
      {trendLabel && (
        <div className="mt-3 text-xs text-slate-500 border-t border-white/5 pt-3">{trendLabel}</div>
      )}
    </div>
  );
}
