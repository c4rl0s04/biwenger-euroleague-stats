  ...
  return (
    <MobileScreen labelledBy="mobile-screen-title">
      ...
      <div className="mobile-compare-hero">
        ...
        <div>
          <strong>Tu temporada contra la suya</strong>
          <span>Puntos, regularidad, plantilla y decisiones</span>
        </div>
      </div>
        ...
        {users
          ...
          .map((user) => (
            <MobileListRow
              ...
              leading={
                <span className="mobile-profile-badge mobile-compare-avatar">
                  {String(user.name).slice(0, 2)}
                </span>
              }
              title={user.name}
              subtitle="Abrir comparación completa"
              trailing="VS"
            />
          ))}
      </div>
