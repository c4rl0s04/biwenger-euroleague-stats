    ...
    >
      <div className="flex flex-col h-full justify-between space-y-4">
        <div className="grid grid-cols-2 gap-6">
          {/* Media Season */}
          <div>
            <div className="text-amber-400 text-xs uppercase tracking-wider mb-2 font-bold">
              Media
            </div>
          ...
          <div className="pt-4 border-t border-slate-700/30 border-l border-slate-700/50 pl-6">
            ...
            <div className="text-2xl font-bold text-emerald-400">
              {(() => {
                ...
                const avg =
                  last3.reduce((sum, m) => sum + (m.fantasy_points || 0), 0) / last3.length;
        ...
        <div className="pt-4 border-t border-slate-700/30 flex items-center justify-between">
          ...
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold border ${player.status === 'ok' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}
          >
            <span
              className={`w-2 h-2 rounded-full ${player.status === 'ok' ? 'bg-emerald-400' : 'bg-rose-400'} animate-pulse`}
            />
