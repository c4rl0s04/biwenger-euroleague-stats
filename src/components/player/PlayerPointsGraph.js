  ...
  if (active && payload && payload.length) {
    ...
    if (mode === 'result') {
      if (isWin) {
        ...
        resultText = 'Victoria';
        resultIcon = <Trophy className="w-3 h-3" />;
      } else {
        resultColor = 'text-red-500';
        resultText = 'Derrota';
        resultIcon = <Ban className="w-3 h-3" />;
      }
    } else {
      if (d.isHome) {
        resultColor = 'text-blue-400';
        resultText = 'Casa';
        resultIcon = <House className="w-3 h-3" />;
      } else {
        resultColor = 'text-purple-400';
        resultText = 'Fuera';
        resultIcon = <Plane className="w-3 h-3" />;
      }
    }
    ...
    return (
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 shadow-xl pointer-events-none">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span
            className={`text-xs px-1.5 py-0.5 rounded ${mode === 'result' ? 'bg-slate-700 text-slate-300' : d.isHome ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}
          >
            {mode === 'result' ? `${d.home_score} - ${d.away_score}` : resultText}
          </span>
        </div>
        <div
          className={`font-bold text-lg ${mode === 'result' && !d.isDNP ? (isWin ? 'text-green-500' : 'text-red-500') : d.isDNP ? 'text-slate-500' : d.isHome ? 'text-blue-400' : 'text-purple-400'}`}
        >
          {d.isDNP ? (
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
          <BarChart data={data} margin={{ top: 10, right: 30, left: -10, bottom: 0 }}>
            ...
            <ReferenceLine
              ...
              strokeDasharray="3 3"
              opacity={0.5}
              label={{ value: 'Avg', position: 'insideRight', fill: '#94a3b8', fontSize: 10 }}
            />
