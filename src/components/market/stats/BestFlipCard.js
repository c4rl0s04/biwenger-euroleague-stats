...
import { Rocket, ChevronDown, ChevronUp, Trophy } from 'lucide-react';
...
export default function BestFlipCard({ flip }) {
  ...
  const winner = flip[0];
  const runnerUps = flip.slice(1, 3); // Only 2nd and 3rd for podium
  const restRunnerUps = flip.slice(3);
  const winnerColor = getColorForUser(winner.user_id, winner.user_name, winner.user_color_index);

  // Find max profit for progress bar calculation
  const maxProfit = Math.max(...flip.map((f) => f.profit));
      ...
      >
        <div className="flex flex-col">
          ...
          <div className="relative mt-2">
            {/* Player Image with Gradient */}
            <div className="flex justify-center mb-3">
              <div className="relative">
                <div className="w-20 h-20 rounded-full overflow-hidden ring-4 ring-emerald-500/50 shadow-lg shadow-emerald-500/20">
                  <PlayerImage
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          ...
                          className="text-xs font-medium text-zinc-200 truncate hover:text-emerald-400 transition-colors block"
                        >
                          {item.player_name}
                        </Link>
          ...
          {/* Expand/Collapse Button for more */}
          {restRunnerUps.length > 0 && (
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className="mt-3 flex items-center justify-center gap-1 text-[10px] text-zinc-500 hover:text-emerald-400 transition-colors py-1 border-t border-zinc-800 cursor-pointer"
            >
              {isExpanded ? (
                <>
          ...
          {/* Rest of Runner-ups - Collapsible */}
          <div
