            ...
            ranking.map((user) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${getPositionStyle(user.value_position)}`}
                  >
                    {user.value_position}
                  </span>
                  {user.icon ? (
                ...
                <div className="text-right">
                  <div
                    className={`font-bold ${user.value_position === 1 ? 'text-emerald-400' : 'text-blue-400'}`}
                  >
                    {formatPrice(user.team_value)}
                  </div>
                  {user.price_trend !== 0 && (
                    <div
                      className={`flex items-center justify-end gap-1 text-xs ${user.price_trend > 0 ? 'text-green-400' : 'text-red-400'}`}
                    >
                      {user.price_trend > 0 ? (
