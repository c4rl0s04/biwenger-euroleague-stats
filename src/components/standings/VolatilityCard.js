...
export default function VolatilityCard() {
  ...
  return (
    <Card title="Montaña Rusa (Volatilidad)" icon={Waves} color="pink" loading={loading} tooltip="Desviación Estándar de los puntos. Valores altos indican rendimiento impredecible; bajos indican consistencia.">
          ...
          {data.slice(0, 8).map((user, index) => {
            ...
            return (
              <div key={user.user_id} className="relative group">
                <Link href={`/user/${user.user_id}`} className={`flex items-center justify-between py-2 px-2 hover:bg-slate-800 rounded transition-colors z-10 relative ${userColor.hover}`}>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-slate-500 font-mono w-4">{index + 1}</span>
                    <span className="text-sm font-medium text-slate-300 group-hover:text-inherit transition-colors">{user.name}</span>
                  </div>
