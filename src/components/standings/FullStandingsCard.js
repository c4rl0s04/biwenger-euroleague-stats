...
export default function FullStandingsCard() {
  ...
  const getPositionStyle = (position) => {
    if (position === 1) return 'bg-yellow-500 text-slate-950';
    if (position === 2) return 'bg-slate-200 text-slate-900';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val, row) => (
        <div className="flex flex-col items-center">
          <span className="text-foreground/90 text-base md:text-lg font-bold leading-tight">
            {val}
          </span>
