...
export default function DominanceCard() {
  ...
  return (
    <Card title="Margen de Victoria (Dominance)" icon={Crown} color="yellow" loading={loading} tooltip="Promedio de puntos de diferencia sobre el 2º clasificado en las jornadas ganadas.">
      {!loading && data.length > 0 ? (
        <div className="flex flex-col gap-2 h-full">
          {data.map((user, index) => {
            const userColor = getColorForUser(user.user_id, user.name);
            return (
              <div key={user.user_id} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/30 hover:bg-slate-800/50 transition-colors w-full">
                {/* User Info */}
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <span className="text-slate-500 font-mono text-xs w-4">#{index + 1}</span>
                  {user.icon ? (
                     <img src={user.icon} alt={user.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-white/10" />
                  ) : (
                  ...
                  <Link href={`/user/${user.user_id}`} className={`font-semibold text-sm text-slate-200 ${userColor.hover} transition-colors truncate`}>
                    {user.name}
                  </Link>
                </div>
                ...
                <div className="text-right min-w-[70px]">
                  <span className="block text-xl font-black text-yellow-400 tracking-tight">+{user.avg_margin.toFixed(1)}</span>
