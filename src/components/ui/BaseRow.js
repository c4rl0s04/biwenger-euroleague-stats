...
/**
 * BaseRow
 * Foundation component for all ranking list items.
 * Supports Manager themes, player images, and complex value layouts.
 */
export default function BaseRow({
  ...
  name,
  linkPath,
  primaryColor = { text: 'text-white' },
  secondaryColor = { text: 'text-zinc-500' },
  managerName,
  managerId,
  valueLabel,
  valueText,
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col">
            ...
            >
              <h4
                className={`font-black uppercase tracking-tight truncate leading-tight transition-colors hover:scale-105 origin-left duration-200 ${primaryColor?.text || 'text-white'}`}
              >
                {name}
              </h4>
            </Link>
              ...
              <div className="mt-1">
                {isUser ? (
                  ...
                  <Link
                    href={`/user/${managerId}`}
                    onClick={(e) => e.stopPropagation()}
                    className={`text-[10px] font-black uppercase tracking-widest hover:scale-105 inline-block origin-left transition-transform duration-200 ${secondaryColor?.text || 'text-zinc-500'}`}
                  >
                    {managerName}
                  </Link>
                ) : (
        ...
        <div className="flex flex-col items-end shrink-0 gap-1">
          ...
          <span
            className={`text-xl font-black tabular-nums transition-colors ${primaryColor?.text || 'text-white'}`}
          >
            {valueText}
          </span>
        </div>
