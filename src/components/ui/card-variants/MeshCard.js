...
export default function MeshCard({
  ...
  if (loading) {
    return (
      <div
        className={`bg-slate-950 border border-slate-800 rounded-2xl p-0 h-full flex flex-col animate-pulse overflow-hidden ${className}`}
      >
        {/* Mimic Header Glow */}
        <div
          className={`h-24 bg-gradient-to-b from-${glowColor || 'indigo-900'}/20 to-transparent opacity-50`}
        ></div>
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 inset-x-0 h-32 bg-${glowColor} blur-[80px] opacity-20 pointer-events-none`}
      ></div>
      ...
      <div className="p-6 h-full flex flex-col relative z-10 backdrop-blur-0">
        <div className="flex items-start justify-between mb-5">
          <div className="flex items-center gap-2.5">
            {icon &&
              createElement(icon, {
                className: `w-5 h-5 opacity-70 group-hover:opacity-100 transition-opacity ${styles.split(' ').pop()}`,
              })}
