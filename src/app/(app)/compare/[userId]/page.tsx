...
export default async function CompareOpponentPage({ params }: PageProps) {
  ...
  const currentStanding = data.standings.find(
    (row: RecordValue) => String(row.user_id) === String(current.id)
  );
  const opponentStanding = data.standings.find(
    (row: RecordValue) => String(row.user_id) === String(opponent.id)
  );
  const currentHistory = data.history.find(
    (entry: RecordValue) => String(entry.userId) === String(current.id)
  );
  const opponentHistory = data.history.find(
    (entry: RecordValue) => String(entry.userId) === String(opponent.id)
  );
