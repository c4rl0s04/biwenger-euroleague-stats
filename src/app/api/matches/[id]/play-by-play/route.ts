...
import { getOfficialPlayByPlay } from '@/lib/db/queries/competition/official-game-data';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await getOfficialPlayByPlay(matchId!, { period, playerId, teamCode });
    if (!result) return errorResponse('Match or official play-by-play not found.', 404);
