...
export default function MobileTeamProfileScreen({ team }: { team: TeamProfileViewModel }) {
  return (
    <MobileScreen labelledBy="mobile-screen-title">
      ...
      <section className="mobile-profile-identity">
        <span className="mobile-profile-badge">EL</span>
        <div>
          <p>Equipo</p>
          <h2>{team.name}</h2>
      ...
      <MobileMetricGrid>
        ...
        <MobileMetric
          label="Propietarios"
          value={new Set(team.roster.map((player) => player.ownerId).filter(Boolean)).size}
          tone="positive"
        />
      </MobileMetricGrid>
      ...
      <div>
        <MobileSectionLink
          ...
          title="Plantilla"
          description="Jugadores, posiciones y propiedad"
          icon={Users}
          accent="green"
        />
        <MobileSectionLink
          ...
          title="Partidos"
          description="Próximos retos y resultados"
          icon={CalendarDays}
        />
      </div>
    </MobileScreen>
