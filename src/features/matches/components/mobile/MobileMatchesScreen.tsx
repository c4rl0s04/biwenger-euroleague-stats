    ...
    <MobileScreen labelledBy="mobile-screen-title">
      ...
      <div className="mobile-control-offset">
        <MobileSegmentedControl
          ...
          items={rounds.map((round) => ({
            label: round.roundName.replace('Jornada ', 'J'),
            href: `/matches?roundId=${round.roundId}`,
            active: round.roundId === activeRound?.roundId,
          }))}
        />
      </div>
      ...
      {activeRound && (
        <>
          ...
          <MobileSectionLink
            href={`/matches/round/${activeRound.roundId}`}
            title="Detalle de jornada"
            description="Partidos y contexto completo"
            icon={MapPinned}
            accent="blue"
          />
        </>
