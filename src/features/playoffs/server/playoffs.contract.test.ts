...
import { userSeasons, playoffPredictions, playoffResults, userPlayoffMedia } from '@/lib/db/schema';
...
it('preserves stage weights, finished denominators, unknown stages and pending outcomes', async () => {
  ...
  expect(mocks.tables).toEqual([userSeasons, playoffPredictions, playoffResults, userPlayoffMedia]);
});
