...
import { decodeHomeFeedCursor } from '@/features/home/server/validation/cursor';
...
vi.mock('server-only', () => ({}));
vi.mock('@/features/home/server/queries/home-feed.query', () => ({
  queryHomeActivityRows,
  queryHomeRoundHighlightPlayers,
}));
vi.mock('@/features/home/server/queries/home-summary.query', () => ({ queryHomeSeasonMetadata }));
vi.mock('@/features/rounds/server', () => ({ getRoundCalendar }));
vi.mock('@/features/managers/server', () => ({ getManagerPersonalizedAlerts }));
vi.mock('@/features/standings/server', () => ({ getRequestStandings: getAppStandings }));

import { getHomeFeedPage } from './feed.service';
import { getHomeSummary } from './summary.service';
...
describe('mobile home feed service', () => {
  ...
  it('does not create an invalid champion link when winner metadata cannot resolve a manager', async () => {
    queryHomeActivityRows.mockResolvedValue([
      {
        id: 'tournament_round:9:40',
        type: 'tournament_round',
        ...
        payload: {
          ...
          tournamentStatus: 'finished',
          isFinalRound: true,
          dataJson: JSON.stringify({ winner: { name: 'Manager desconocido', icon: 'raw.png' } }),
          fixtures: [
            {
              id: 44,
              homeUserId: '7',
              homeName: 'All Stars',
              homeIcon: 'normalized.png',
              homeColorIndex: 2,
              homeScore: 185,
              awayUserId: '3',
              awayName: 'June',
  ...
  it('keeps an empty configured season in preseason without historical fallback', async () => {
    ...
    getRoundCalendar.mockResolvedValue({ currentRound: null, nextRound: null });
  ...
  it('maps the active round and personal pulse from shared standings', async () => {
    ...
    getRoundCalendar.mockResolvedValue({
      currentRound: {
        roundId: 4,
        roundName: 'Jornada 4',
        status: 'live',
