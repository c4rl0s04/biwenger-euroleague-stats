...
import { queryHomeActivityRows, queryHomeRoundHighlightPlayers } from './home-feed.query';
...
describe('home activity feed query', () => {
  ...
  it('binds the selected season and keeps the timestamp/ID cursor ordering together', async () => {
    resolveReadSeasonId.mockResolvedValue('historical-season');
    const cursor = {
      occurredAt: '2026-10-01T00:00:00Z',
      id: 'admin_bonus:2',
      filter: 'all' as const,
    };
    await queryHomeActivityRows({ filter: 'all', cursor, limit: 16 });
    const [sql, params] = query.mock.calls[0];
    expect(params).toEqual(['historical-season', cursor.occurredAt, cursor.id, null, 16]);
    expect(sql).toContain('OR (occurred_at = $2::timestamptz AND id < $3::text)');
    expect(sql).toContain('ORDER BY occurred_at DESC, id DESC');
    expect(sql).toContain('LIMIT $5');
    expect(sql).toContain('WHERE season_id = $1');
  });

  it('does not resolve season or query for an empty highlight batch', async () => {
    expect(await queryHomeRoundHighlightPlayers([])).toEqual([]);
    expect(resolveReadSeasonId).not.toHaveBeenCalled();
