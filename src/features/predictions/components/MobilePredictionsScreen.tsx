  ...
  return (
    <MobileScreen labelledBy="mobile-screen-title">
      ...
      <MobileMetricGrid>
        <MobileMetric
          label="Líder"
          value={leader?.usuario ?? '—'}
          detail={`${leader?.total_aciertos ?? 0} aciertos`}
          tone="accent"
        />
        <MobileMetric
          label="Más victorias"
          value={victories?.usuario ?? '—'}
          detail={`${victories?.victorias ?? 0} jornadas`}
        />
      ...
      <div>
        {ranking.slice(0, 7).map((row: Stats, index: number) => (
          <MobileListRow
            ...
            leading={<span className={`mobile-rank mobile-rank-${index + 1}`}>{index + 1}</span>}
            title={row.usuario}
            subtitle={`${row.jornadas_jugadas} jornadas`}
            trailing={row.total_aciertos}
          />
        ))}
      </div>
      ...
      <div>
        <MobileSectionLink
          href="/predictions/evolution"
          title="Evolución"
          description="Aciertos jornada a jornada"
          icon={ChartSpline}
          accent="blue"
        />
        <MobileSectionLink
          href="/predictions/ranking"
          title="Ranking"
          description="Clasificación completa y promedios"
          icon={Trophy}
        />
        <MobileSectionLink
          href="/predictions/teams"
          title="Equipos"
          description="Qué equipos son más previsibles"
          icon={Users}
          accent="green"
        />
        <MobileSectionLink
          ...
          title="Historial"
          description="Detalle de todas las porras"
          icon={History}
          accent="violet"
        />
      </div>
