  ...
  return (
    <Link href={`/tournaments/${tournament.id}`} className="block group/item">
      <ElegantCard
        hideHeader
        padding="p-4"
        color={isActive ? 'amber' : 'zinc'}
        bgColor={isActive ? 'amber' : null}
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 flex-1 min-w-0">
            <div
              className={`p-3 rounded-lg border backdrop-blur-sm transition-transform duration-300 group-hover/card:scale-110 ${
                isActive
            ...
            <div className="flex flex-row items-center gap-3 min-w-0">
              <h3 className="text-xl font-black font-display tracking-tight text-white group-hover/card:text-primary transition-all truncate shrink-0 max-w-[50%]">
                {tournament.name}
              </h3>
          ...
          <div className="flex items-center gap-4 shrink-0">
            {!isActive && data.winner ? (
              ...
              <div
                className={`hidden sm:flex items-center gap-2 text-xs font-medium transition-colors ${
                  isActive
                    ? 'text-amber-400/80 group-hover/card:text-amber-400'
                    : 'text-zinc-500 group-hover/card:text-zinc-300'
                }`}
              >
                <Users size={14} />
            ...
            <ArrowRight
              ...
              className={`transition-transform duration-300 group-hover/card:translate-x-1 ${
                isActive ? 'text-amber-500' : 'text-zinc-600 group-hover/card:text-white'
              }`}
            />
          </div>
        </div>
