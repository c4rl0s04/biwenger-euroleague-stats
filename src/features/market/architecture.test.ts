...
it('types aggregate and chart reads without adding server imports or cache changes', () => {
  const parent = feature('screens/components/MarketPageClient.tsx');
  expect(parent).toContain('Partial<MarketAnalytics>');
  expect(parent).toContain('useState<MarketDrawerState>');
  const chart = feature('trends/components/MarketTrendsChart.tsx');
...
it('owns typed listing composition and consumes the deliberate Players HTTP model', () => {
  expect(feature('catalogue/models/market-listing-presentation.ts')).toContain(
    "from '@/features/players/public'"
  );
  expect(feature('catalogue/models/market-listing-presentation.ts')).not.toMatch(
    /\bany\b|drizzle|queries|ReturnType/
  );
  expect(feature('catalogue/components/MarketListingsSection.tsx')).toContain(
    'listings?: CurrentMarketListing[]'
  );
  for (const name of ['MarketPlayerCard', 'ExpandedPlayerModal']) {
    const source = feature(`catalogue/components/${name}.tsx`);
...
it('keeps metric renderer inputs explicit and the shared visual facade implementation-free', () => {
  expect(feature('analytics/models/market-metric.ts')).not.toMatch(
    /\bany\b|drizzle|queries|ReturnType/
  );
  for (const name of ['PlayerStatRow', 'UserStatRow', 'TransactionStatRow', 'TemporalStatRow']) {
    expect(feature(`analytics/components/renderers/${name}.tsx`)).toContain('MarketMetricRowProps');
  }
  expect(feature('analytics/components/renderers/registry.tsx')).toContain(
    'MarketMetricDefinition'
  );
  expect(feature('analytics/components/renderers/utils.ts')).toContain('MarketRowIdentity');
  expect(feature('analytics/components/renderers/BaseRow.ts')).toContain(
    'ComponentType<MarketBaseRowProps>'
  );
  expect(feature('analytics/components/renderers/BaseRow.ts')).not.toContain('function');
});
...
it('owns the section composition and typed client-local drawer boundary', () => {
  ...
  expect(feature('screens/components/MarketSectionScreen.tsx')).toContain('MobileDetailScaffold');
  const model = feature('analytics/models/market-drawer.ts');
  expect(model).not.toMatch(/\bany\b|drizzle|queries|ReturnType/);
  expect(model).toContain('MarketDrawerRowsByType');
  expect(feature('analytics/components/StatDetailDrawer.tsx')).toContain('}: MarketDrawerProps)');
  expect(feature('screens/components/MarketPageClient.tsx')).toContain('MarketDrawerConfig');
});
...
it('owns non-bids phone projections and enforces the section route without a new exception', () => {
  ...
  expect(feature('screens/models/market-section.ts')).not.toMatch(
    /\bany\b|\bDate\b|drizzle|ReturnType/
  );
  expect(feature('screens/components/MarketSectionRows.tsx')).not.toMatch(
    /MobileRecordList|@\/lib\/(db|services)/
  );
  expect(feature('screens/server/services/market-section.service.ts')).toMatch(
    /^import 'server-only';/
  );
...
it('keeps the public contract and trend models independent from server and database code', () => {
  ...
  expect(feature('trends/models/market-trends.ts')).not.toMatch(
    /\bany\b|\bDate\b|drizzle|ReturnType/
  );
  expect(feature('server.ts')).toMatch(/^import 'server-only';/);
  expect(feature('trends/server/queries/market-trends.query.ts')).toMatch(/^import 'server-only';/);
  expect(feature('trends/server/services/market-trends.service.ts')).toMatch(
    /^import 'server-only';/
  );
  expect(feature('trends/server/services/market-trends.service.ts')).not.toMatch(
...
it('keeps transfer/detail services and models off legacy database-shaped boundaries', () => {
  expect(feature('trends/models/market-transfers.ts')).not.toMatch(
    /\bany\b|\bDate\b|drizzle|ReturnType/
  );
  expect(feature('trends/server/services/market-transfers.service.ts')).toMatch(
    /^import 'server-only';/
  );
  expect(feature('trends/server/services/market-transfers.service.ts')).not.toMatch(
    /@\/lib\/(db|services)|drizzle/
  );
  expect(feature('trends/server/queries/market-transfers.query.ts')).toMatch(
    /^import 'server-only';/
  );
  expect(feature('trends/server/mappers/market-transfers.mapper.ts')).not.toMatch(
    /\.\.\.row|@\/lib\/db/
  );
  const policy = JSON.parse(read('scripts/architecture/policy.json'));
...
it('owns catalogue orchestration with deliberate cross-feature contracts and allowlisted models', () => {
  const service = feature('catalogue/server/services/market-catalogue.service.ts');
  ...
  const query = feature('catalogue/server/queries/market-catalogue.query.ts');
  expect(query).toMatch(/^import 'server-only';/);
  expect(query).not.toMatch(/features\/players|features\/teams|services\//);
  expect(feature('catalogue/models/market-catalogue.ts')).not.toMatch(
...
it('owns every remaining analytics query, projection and stats HTTP boundary', () => {
  ...
  for (const group of ['summary', 'auctions', 'investments', 'overview']) {
    expect(feature(`analytics/models/market-${group}.ts`)).not.toMatch(
      /\bany\b|\bDate\b|drizzle|queries/
    );
    expect(feature(`analytics/server/queries/market-${group}.query.ts`)).toMatch(
      /^import 'server-only';/
    );
    expect(feature(`analytics/server/mappers/market-${group}.mapper.ts`)).not.toMatch(
      /\.\.\.row|\bany\b/
    );
    expect(feature(`analytics/server/services/market-${group}.service.ts`)).not.toMatch(
      /@\/lib\/(db|services)|\bany\b/
    );
  }
  for (const group of ['activity-extra']) {
    expect(feature(`trends/models/market-${group}.ts`)).not.toMatch(
      /\bany\b|\bDate\b|drizzle|queries/
    );
    expect(feature(`trends/server/queries/market-${group}.query.ts`)).toMatch(
      /^import 'server-only';/
    );
    expect(feature(`trends/server/mappers/market-${group}.mapper.ts`)).not.toMatch(
      /\.\.\.row|\bany\b/
    );
    expect(feature(`trends/server/services/market-${group}.service.ts`)).not.toMatch(
      /@\/lib\/(db|services)|\bany\b/
    );
  }
...
it('owns basic activity orchestration and registers its HTTP boundary', () => {
  expect(feature('trends/models/market-activity.ts')).not.toMatch(
    /\bany\b|\bDate\b|drizzle|ReturnType/
  );
  expect(feature('trends/server/services/market-activity.service.ts')).toMatch(
    /^import 'server-only';/
  );
  expect(feature('trends/server/services/market-activity.service.ts')).not.toMatch(
    /@\/lib\/(db|services)|drizzle/
  );
  expect(feature('trends/server/queries/market-activity.query.ts')).toMatch(
    /^import 'server-only';/
  );
