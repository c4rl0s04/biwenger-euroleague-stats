...
import type { MarketActivityTransfer } from '../models/market-activity';
import type { SingleFlip } from '../models/market-investments';
const fake = vi.hoisted(() => ({ transfers: vi.fn(), trends: vi.fn(), stats: vi.fn() }));
vi.mock('./services/market-activity.service', () => ({ getAllTransfers: fake.transfers }));
vi.mock('./services/market-trends.service', () => ({ getMarketTrendsAnalysis: fake.trends }));
vi.mock('./services/market-analytics.service', () => ({ fetchMarketStats: fake.stats }));
