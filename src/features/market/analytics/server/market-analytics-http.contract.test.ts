...
import { MARKET_ANALYTICS_POLICY } from '@/features/market/server';
...
beforeEach(() => {
  ...
  fake.query.mockImplementation(async (sql: string) => {
    ...
    if (
      sql.includes('ORDER BY us.name ASC, us.user_id ASC') ||
      sql.includes('ORDER BY COALESCE(us.name, u.name) ASC, u.id ASC')
    )
      return {
        rows: [
          {
