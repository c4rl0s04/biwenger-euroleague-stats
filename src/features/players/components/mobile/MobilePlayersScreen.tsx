    ...
    <MobileScreen labelledBy="mobile-screen-title">
      ...
      <div className="mobile-player-toolbar">
        ...
        <button
          ...
          className={position ? 'mobile-filter-active' : undefined}
        >
      ...
      <div>
        {players.slice(0, 60).map((player) => (
          <MobileListRow
            ...
            trailing={
              <span className="mobile-player-points">
                {Number(player.avg_points ?? player.season_avg ?? 0).toLocaleString('es-ES')}
              </span>
            }
          />
        ))}
      ...
      <div>
        <MobileSectionLink
          href="/players/insights"
          title="Insights"
          description="Forma, valor y oportunidades"
          icon={Sparkles}
        />
        <MobileSectionLink
          href="/players/squads"
          title="Plantillas"
          description="Distribución por mánager y posición"
          icon={Users}
          accent="blue"
        />
      </div>
      ...
      >
        <form onSubmit={applyFilters} className="mobile-filter-form">
          ...
          <button type="submit" className="mobile-primary-action">
            <Filter size={18} aria-hidden="true" /> Aplicar filtros
          </button>
        </form>
      </MobileBottomSheet>
    </MobileScreen>
