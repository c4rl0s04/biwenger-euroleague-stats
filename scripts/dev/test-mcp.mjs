...
async function main() {
  ...
  try {
    ...
    if (
      glossaryText.includes('Valoración') &&
      glossaryText.includes('Fichaje') &&
      glossaryText.includes('Saldo')
    ) {
      console.log(
        '   ✅ PASS: El glosario contiene las definiciones clave extendidas (Finanzas, Mercado).'
      );
    } else {
    ...
    for (const player of playersToSearch) {
      ...
      if (
        searchOutput.includes(player) &&
        searchOutput.includes('Media:') &&
        searchOutput.includes('Últimos 5:')
      ) {
    ...
    if (
      detailsOutput.includes('PERFIL') &&
      detailsOutput.includes('FORMA') &&
      detailsOutput.includes('PRÓXIMOS RIVALES')
    ) {
    ...
    if (
      sqlOutput.includes('Vezenkov') ||
      sqlOutput.includes('James') ||
      sqlOutput.includes('price')
    ) {
